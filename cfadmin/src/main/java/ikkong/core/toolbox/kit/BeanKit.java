...
import ikkong.core.toolbox.Conver;
...
public class BeanKit {
	...
	public static <T> T fillBeanWithRequestParam(final javax.servlet.ServletRequest request, T bean){
		final String beanName = JStrKit.lowerFirst(bean.getClass().getSimpleName());
		return fill(bean, new ValueProvider(){
			@Override
			public Object value(String name) {
				String value = request.getParameter(name);
				if (JStrKit.isBlank(value)) {
					//使用类名前缀尝试查找值
					value = request.getParameter(beanName + JStrKit.DOT + name);
					if(JStrKit.isBlank(value)){
						//此处取得的值为空时跳过，包括null和""
						value = null;
					}
		...
		try {
			...
			for (PropertyDescriptor property : propertyDescriptors) {
				...
				try {
					...
					throw new ToolBoxException(JStrKit.format("Inject [{}] error!", property.getName()), e);
				}
			}
		} catch (Exception e) {
