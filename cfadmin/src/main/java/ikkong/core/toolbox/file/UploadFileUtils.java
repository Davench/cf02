package ikkong.core.toolbox.file;
...
import ikkong.core.constant.Cst;
...
import ikkong.core.toolbox.kit.DateKit;
import ikkong.system.model.Attach;
...
public class UploadFileUtils {
	...
	// 图片扩展名
	private static String[] fileTypes = new String[] { "gif", "jpg", "jpeg",
			"png", "bmp" };
	...
	public static boolean testExt(String dir, String fileName) {
		...
		if (JStrKit.isBlank(ext) || ext.indexOf(fileExt) == -1) {
			return false;
		}
		return true;
	}
	...
	public static Map<String, Object> listFiles(String dir, String path,
			String order) {
		...
		String rootUrl = Cst.me().getContextPath() + File.separator + saveDir
				+ File.separator;
	...
	public static String uploadBase64(String nr, String addr) {
		...
		try {
			...
			try {
				...
				String newFileName = DateKit.getAllTime() + "_"
						+ new Random().nextInt(1000) + "." + addr;
	...
	public static String uploadMultiFile(UploadFile mfile) {
		String contentPath = "";
		try {
			String nowdate = DateKit.getDays();
			String path = Cst.me().getUploadRealPath();
			...
			String newFileName = DateKit.getAllTime() + "_"
					...
					+ getFileExt(mfile.getOriginalFileName());
			String uploadPath = path + "/" + nowdate + "/" + newFileName;
			contentPath = contentPath + "/" + nowdate + "/" + newFileName;
			if (!file.exists() && !file.isDirectory()) {
				file.mkdirs();
			...
			File imgfile = new File(uploadPath);
			forChannel(mfile.getFile(), imgfile);
//			mfile.transferTo(imgfile);
	...
	public static void forChannel(File s, File t) {
	...
	private static String getUploadPath(String originalFileName){
		return Cst.me().getUploadRealPath() + File.separator + DateKit.getDays() + File.separator + originalFileName;
	}
	
	
	
	public static void transfer(UploadFile file){
		try {
			forChannel(file.getFile(),new File(getUploadPath(file.getOriginalFileName())));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
